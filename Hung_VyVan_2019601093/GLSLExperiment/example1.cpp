...
GLfloat goc_quay_x_may_cau = 30, goc_quay_y_may_cau = 30, goc_quay_z_may_cau = 0, goc_quay_banh_xe_may_cau = 0, goc_quay_truc_cau = -45;
GLfloat dich_x_may_cau = 0, dich_y_may_cau = 0, dich_z_may_cau = 0; // dịch xyz
GLfloat goc_quay_canh_cua_may_cau = 0;

GLfloat ti_le_may_cau = 0.2;   // chiều dài tượng trưng cho sàn máy cẩu
...
void keyboard(unsigned char key, int x, int y)
{
	...
	case 'A':
		...
		cout << "xe dang chay lui ve phia sau" << endl;
		glutPostRedisplay();
		break;
	case 'a':
		dich_x_may_cau += cd_may_cau / 30;
